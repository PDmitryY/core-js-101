...
function doRectanglesOverlap(rect1, rect2) {
  let result = true;
  if (rect1.top > rect2.top + rect2.height) { result = false; }
  if (rect1.left > rect2.left + rect2.width) { result = false; }
  if (rect1.top + rect1.height < rect2.top) { result = false; }
  if (rect1.left + rect1.width < rect2.left) { result = false; }
  return result;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < ccnString.length; i += 1) {
    if (ccnString.length % 2 !== i % 2) sum += ccnString[i] * 1;
    else sum += ccnString[i] * 2 + (ccnString[i] > 4 ? -9 : 0);
  }
  return sum % 10 === 0;
}
